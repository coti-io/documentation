...
| **MPC Executor** | The **COTI-side contract** your integration targets for a given network (see SDK presets such as `PodUserSepolia` in [Getting started](tutorial-private-adder-sepolia.md)). The **Inbox (COTI)** invokes it; it is **not** the same contract as either Inbox. |
