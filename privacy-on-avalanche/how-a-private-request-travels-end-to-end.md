...
| **MPC Executor** | The **COTI-side contract** your integration targets for a given network (see SDK presets such as `PodUserFuji` in [Getting started](https://github.com/coti-io/coti-sdk-pod/tree/main/site/04-getting-started)). The **Inbox (COTI)** invokes it; it is **not** the same contract as either Inbox. |
...
**Mermaid quirk:** on a `box` line, do not put **ASCII** parentheses in the title after `rgb(r, g, b)` (for example `(COTI)`). The parser treats everything from `rgb(` through the **last** `)` on that line as the color string, which breaks the fill. Use an em dash in the box title instead—`(Avalanche Fuji)` / `(COTI)` still appear on participant labels below.
...
sequenceDiagram
    ...
    box rgb(28, 32, 44)  Avalanche Fuji
        participant Dapp as Your Fuji dApp contract
...
- **[Interactive PoD architecture on Fuji (pod.coti.io/avalanche)](https://pod.coti.io/avalanche/index.html)** — Walk through the Fuji ↔ COTI path as an animated demo with contract links and fee consumption.
