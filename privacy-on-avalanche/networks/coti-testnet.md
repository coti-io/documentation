...
COTI Testnet is the **private execution** side of Privacy on Demand. Host-chain dApps on [Avalanche Fuji](fuji.md) send encrypted work through the cross-chain Inbox; COTI runs the private computation and returns encrypted results.
...
> **Portal labeling:** The mother / pToken path above is for **PoD cross-chain Privacy Portal** (host chain Fuji ↔ COTI). For the **native COTI Privacy Portal** (PrivateERC20 on COTI only), see [COTI Privacy Portal — contract addresses](../../coti-privacy-portal/developer-guide/contract-addresses.md).
